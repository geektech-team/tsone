# 对单个包：升级版本 + 构建 + 发布（全流程）
bun run release --package tsone --bump minor --publish
bun run release --package one-chart --bump minor --publish
bun run release --package backone-utils --bump minor --publish
bun run release --package one --bump minor --publish
