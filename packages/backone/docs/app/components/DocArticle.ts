...
export class DocArticle extends Component<DocArticleProps> {
  ...
  protected render(): VNode {
    ...
    return {
      tag: 'article',
      props: { className: 'backone-doc-article' },
      children: [
        {
          tag: 'p',
          props: { className: 'backone-docs-section-label' },
  ...
  private renderBlock(block: BackOneDocBlock): VNode {
    ...
    switch (block.type) {
      ...
      case 'callout':
        return {
          tag: 'aside',
          props: {
            className: `backone-docs-callout backone-docs-callout--${block.kind}`,
          },
          children: [
      ...
      case 'api-table':
        return {
          tag: 'div',
          props: { className: 'backone-docs-api-scroll' },
          children: [
            {
              tag: 'table',
              props: { className: 'backone-docs-api-table' },
              children: [
  ...
  private renderInline(content: BackOneDocInline[]): Array<VNode | string> {
    const { locale } = this.props;
...
function assertNever(value: never): never {
  throw new Error(`Unsupported BackOne docs content: ${String(value)}`);
}
