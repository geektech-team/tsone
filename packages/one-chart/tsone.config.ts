...
// One Chart 文档是「多语言 × 多页面」的静态站：每个 (locale, page) 是一个独立路由，
...
export default defineConfig({
  ...
  build: {
    outDir: 'docs/dist',
    // 部署在子路径时（如 GitHub Pages 的 /tsone/one-chart/）用 TSONE_BASE_PATH 覆盖。
    basePath: process.env.TSONE_BASE_PATH ?? '',
    directoryPages: true,
  },
  // npm 库打包：`tsone build --library` 使用，产出 dist/ 的 ESM bundle 与 .d.ts。
  // 先编译框架 tsone 的类型声明（one-chart 的 tsconfig.build.json 经 paths 引用其产物）。
  library: {
    entry: 'lib/index.ts',
