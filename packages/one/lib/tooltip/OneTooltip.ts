...
import { TooltipBubble, type TooltipBubbleProps } from './TooltipBubble';
import type { OneTooltipProps, OneTooltipTrigger } from './types';

interface OneTooltipState {
...
const TRIGGERS: readonly OneTooltipTrigger[] = [
  'hover-focus',
  'click',
  'manual',
];

export class OneTooltip extends Component<OneTooltipProps, OneTooltipState> {
  private readonly positioner = new OneFloatingPositioner();
  private controller: OneOverlayMountController<TooltipBubbleProps> | undefined;
  ...
  private describedBySaved = false;
  private tooltipId: string | undefined;
  private hovered = false;
  private focused = false;
  private openTimer: number | undefined;
  private closeTimer: number | undefined;

  protected initState(): OneTooltipState {
  ...
  protected onMounted(): void {
    ...
    this.controller = new OneOverlayMountController<TooltipBubbleProps>(
  ...
  private createBubble(context: OneOverlayFactoryContext) {
    ...
    return {
      update: (next: Partial<TooltipBubbleProps>) => bubble.setProps(next),
      destroy: () => {
        bubble.unmount();
        this.restoreDescribedBy();
  ...
  private bubbleOptions(id = this.tooltipId ?? ''): TooltipBubbleProps {
  ...
  private applyDescribedBy(id: string): void {
    ...
    const tokens = (this.previousDescribedBy ?? '')
      .split(/\s+/)
      .filter(Boolean);
  ...
  private normalizedTrigger(): OneTooltipTrigger {
    return TRIGGERS.includes(this.props.trigger as OneTooltipTrigger)
      ? (this.props.trigger as OneTooltipTrigger)
      : 'hover-focus';
  }
}
...
function defaultChildren(
  ...
  children.forEach((child) => {
    ...
    result.push(normalized as VNode);
  });
  return result;
}
