...
export interface MiniProgramDevServer {
  hostname: string;
  port: number;
  outDir: string;
  dispose: () => void;
...
/**
 * 小程序开发模式：把 TSone 应用编译为微信小程序工程并写入
 * dist/dev/mp-wx，watch 模式下源文件变化自动重建（不启动 HTTP 服务）。
 */
export async function startMiniProgramDev(
  options: StartDevServerOptions = {}
): Promise<MiniProgramDevServer> {
  const watch = options.watch ?? false;
  const config = await resolveConfig({ ...options, mpWeixin: true });
  if (!config.mp) {
    throw new Error('Mini program dev requires config.mp or --mp-weixin');
  }
  const outDir = resolve(config.root, 'dist', 'dev', 'mp-wx');
  await assertSafeSubdirectory(
    config.root,
    outDir,
    INVALID_DEVELOPMENT_DIRECTORY_MESSAGE
  );
  const devConfig: ResolvedConfig = { ...config, mp: { ...config.mp, outDir } };

  const rebuild = async (): Promise<void> => {
    const { buildMiniProgram } = await import('./mp/build-mp');
    const result = await buildMiniProgram(devConfig);
    console.log(
      `TSone mini program dev build: ${result.outDir} (${result.assetsBuilt.length} assets)`
    );
  };
  ...
  if (watch) {
    const mpBuildOutDir = config.mp.outDir;
    watcher = createFileWatcher({
      root: config.root,
      isRelevant: (relativePath) =>
        isRelevantMiniProgramPath(
          config.root,
          mpBuildOutDir,
          outDir,
          relativePath
      ...
      onChange: () => {
        rebuild().catch((error) => {
          console.error(
            'TSone mini program rebuild failed; keeping the last build.'
          );
...
function isRelevantMiniProgramPath(
  root: string,
  buildOutDir: string,
  devOutDir: string,
  relativePath: string
  ...
  if (isWithinOrEqual(buildOutDir, absolutePath)) {
    return false;
  }
  if (isWithinOrEqual(devOutDir, absolutePath)) {
    return false;
  }
