...
export const enBenchmarkPages: DocPage[] = [
  {
    ...
    body: [
      ...
      list([
        ...
        [
          inlineCode('toggle1000'),
          ': toggle the first 1000 items (single commit)',
        ],
        [
          inlineCode('remove1000'),
          ': remove the first 1000 items (single commit)',
        ],
        [
          inlineCode('toggleEach1000'),
          ': toggle 1000 items one by one (one independent update each, to observe batching behavior)',
        ],
      ]),
      ...
      table(
        ...
        [
          ['Mount (1000 items)', '19.9', '43.0', '62.0'],
          ['add1000', '16.6', '11.5', '10.1'],
          ['toggle1000', '18.8', '6.5', '5.4'],
          ['remove1000', '18.8', '6.1', '5.1'],
          ['toggleEach1000 (one by one × 1000)', '8.1', '4.6', '10.2'],
        ]
      ),
      heading(2, 'Interpretation'),
      heading(3, 'Bundle Size: A Clear Win for TSone'),
      paragraph(
        'TSone is 11.1 KB after gzip — about 1/5.3 of React (59.2 KB) and 1/3.3 of Vue (36.8 KB). The 0.4 KB increase over the previous run (10.7 KB) corresponds to newly added runtime capabilities: watch, Transition, KeepAlive, error boundaries, and router guards. The smaller runtime pays off on slow networks, low-end devices, and first-load scenarios.'
      ),
      heading(3, 'Mount Time: Same Order of Magnitude'),
      paragraph(
        'Mounting 1000 items lands in the 20-62 ms range for all three frameworks, with TSone the fastest. React is relatively higher, mostly due to runtime initialization and module parsing.'
      ),
      heading(3, 'Single Batch Updates: Slower but Imperceptible'),
      paragraph(
        'One-shot bulk operations stay below 20 ms in all three frameworks. TSone is roughly 1.5-3.7× slower than Vue/React (about 1.5× for add1000, about 3× for toggle/remove) because every update rebuilds and diffs the full list of VNodes, whereas Vue/React benefit from optimized keyed rendering.'
      ),
      heading(3, 'Fine-Grained Updates: Automatic Batching Closes the Gap'),
      paragraph(
        'Updating 1000 items one by one (toggleEach1000) takes about 8.1 ms in TSone, in the same league as Vue (~4.6 ms) and React (~10.2 ms), and down from 9.7 ms in the previous run thanks to value-level props diffing and batched setProps. TSone ships a built-in automatic batching scheduler: a burst of synchronous state changes is merged into a single render on a microtask. Before this optimization the effect ran synchronously and re-rendered on every change, putting this metric at about 12 seconds; automatic batching improves it by roughly 1200×, matching Vue\u2019s microtask scheduler and React\u2019s automatic batching.'
      ),
      paragraph(
      ...
      table(
        ['Item', 'Value'],
        [
          ['TSone', '0.4.0 (workspace source, with automatic batching)'],
          ['Bun', '1.4.0'],
