...
export const apiPages: DocPage[] = [
  {
    ...
    body: [
      ...
      paragraph(
        '在 Bun/Node 静态生成环境中，请先提供 DOM-like document，例如项目文档构建脚本使用的 Happy DOM。'
      ),
      codeBlock(
        'ts',
        [
      ...
      list([
        [inlineCode('lang'), ': 输出到 html 元素的语言标记，默认 en'],
        [inlineCode('title'), ': 文档标题，会进行 HTML 转义'],
        [inlineCode('description'), ': 可选 meta description'],
        [
          inlineCode('styles'),
          ': StyleSheet 对象数组，由框架序列化为 style 标签',
        ],
      ...
      paragraph(
        'build.outDir 必须是项目根目录内部的安全子目录。CLI 首版配置不提供 plugins、WebSocket、HMR、SSR、函数式配置、public/ 复制以及公开的 minify/sourcemap 配置。'
      ),
    ],
  },
  {
    ...
    body: [
      ...
      apiTable([
        ...
        },
        {
          ...
          description: '检查值是否为 ref 引用。',
        },
        {
          name: 'unref',
          signature: 'unref<T>(value: T | Ref<T>): T',
          description: '如果传入 ref 则返回 value，否则返回原值。',
        },
        {
          name: 'readonly',
