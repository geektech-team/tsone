 ...
 *
 * 在仓库根目录使用：
 *   bun run release --package <tsone|one|one-chart|tsone-cli> [操作]...
 *
...
function printHelp(): void {
  console.log(
    [
      '用法：bun run release --package <tsone|one|one-chart|tsone-cli> [操作]...',
      '',
      ...
      '  bun run release --package tsone --bump major --dry-run',
    ].join('\n')
  );
}
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case '--pkg': {
        ...
        if (!args.pkg) {
          fail(`未知包：${value}（可选 tsone | one | one-chart | tsone-cli）`);
        }
        break;
      }
      case '--build':
...
function main(): void {
  ...
  if (!args.pkg) {
    fail('请用 --package 指定目标包（tsone | one | one-chart | tsone-cli）');
  }
